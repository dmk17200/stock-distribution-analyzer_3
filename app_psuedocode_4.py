...
def calculate_tsmom(price_data, windows=[20, 60, 126, 252]):
    """
...
def analyze_stock_technical(ticker, df):
    ...
    try:
        ...
        # 1. MA position (backtest: ABOVE_BOTH 55.1% up, BELOW_BOTH 47.8%)
        if ma_position == "ABOVE_BOTH":
            bullish_score += 2
        elif ma_position == "BELOW_BOTH":
            bearish_score += 2
        
        # 2. Golden/Death cross (backtest: 54.2% vs 49.4%)
        ...
        # 3. TSMOM alignment (backtest: best single predictor, IC=0.084)
        ...
        # --- Informational warnings (not scored, just displayed) ---
        if volume_context == "EXTREMELY_EXTENDED":
            warning_flags.append(f"EXTREMELY_EXTENDED_FROM_VALUE_{percent_from_node:.0f}%")
        elif volume_context == "EXTENDED":
            warning_flags.append(f"EXTENDED_FROM_VALUE_{percent_from_node:.0f}%")
        
        if z_score > 5:
            warning_flags.append(f"HIGH_Z_SCORE_{z_score:.1f}σ")
        elif z_score < -5:
            warning_flags.append(f"LOW_Z_SCORE_{z_score:.1f}σ")
        
        if is_parabolic:
            warning_flags.append(f"PARABOLIC_{'UP' if acceleration_rate > 0 else 'DOWN'}_{abs(acceleration_rate):.1%}")
        
        if has_sweep:
            warning_flags.append(f"LIQUIDITY_SWEEP_{sweep_direction}")
        ...
        # Override: don't call STRONG_SELL on confirmed uptrends
        if signal in ["STRONG_SELL", "SELL"] and ma_position == "ABOVE_BOTH" and momentum_state in ["FULL_BULL", "LEANING_BULL"]:
            ...
            warning_flags.append("BEARISH_SCORE_BUT_TREND_INTACT")
        
        
        # Override logic for specific dangerous situations
        # STRONG_SELL requires bearish trend confirmation
        # If stock is above both MAs with bullish TSMOM, it's an uptrend that's extended
        # — not a sell, but a "don't chase" / wait for pullback
        if "MULTIPLE_BUBBLE_CONDITIONS_EXTREME_RISK" in warning_flags:
            if ma_position == "ABOVE_BOTH" and momentum_state in ["FULL_BULL", "LEANING_BULL"]:
                signal = "WAIT_FOR_PULLBACK"
                warning_flags.append("BUBBLE_CONDITIONS_BUT_TREND_INTACT")
            else:
                signal = "STRONG_SELL"
                ...
                if stock_fig is None or stock_metrics is None:
                    ...
                    if tech_analysis:
                        ...
                        with col1:
                            ...
                            stock_data_sorted = stock_df.sort_values('date')
                            today_price  = tech_analysis['current_price']
                            today_volume = stock_data_sorted['volume'].iloc[-1]
                            avg_volume   = stock_data_sorted['volume'].tail(20).mean()
                            vol_vs_avg   = today_volume / avg_volume if avg_volume > 0 else 0

                            st.write(f"**Today's Price:** ${today_price:,.2f}")
                            st.write(f"**Today's Volume:** {today_volume:,.0f} ({vol_vs_avg:.2f}x avg)")
...
with tab3:
    ...
    if report_files:
        ...
        ordered_columns_with_repeats = [
            ...
            'price_change_1d',             # previous dailyprice
            'price_change_1d_pct',          # change in daily price
        ...
        rename_map_with_repeats = {
            ...
            'relative_std_dev': 'Rel Std Dev',
            'relative_vol_ratio': 'Vol/Avg Vol',
            'signal': 'Signal',
            'tsmom_20d':       'TSMOM 20d',
            'tsmom_60d':       'TSMOM 60d',
        ...
        def display_sortable_dataframe(display_df, numeric_df, height=400, key_prefix="df"):
            ...
            tsmom_pct_columns  = ['TSMOM 20d', 'TSMOM 60d', 'TSMOM 126d', 'TSMOM 252d']
